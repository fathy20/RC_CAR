...
static void (*TIMER1_PFUNC)(void) = 0 ;		// pointer to functions to be used in ISR
...
void TIMER1_VidInitializeMode15(u8 Mode,u8 ModelA,u8 ModelB){
	switch(Mode){
	   case  FAST_PWM_ICR1 :			// ICR1 ser as top, OCR1x and TOV1 flag will update on reaching ICR1 //
		   CLR_BIT(TCCR1A,WGM10);		// value putted in ICR1A was 19999 // 		// 	MODE 14 //
		   SET_BIT(TCCR1A,WGM11);
		   SET_BIT(TCCR1B,WGM12);
		   SET_BIT(TCCR1B,WGM13);
	       break                ;
	   case  FAST_PWM_OCR1A :			//  OCR1 set as top, OCR1x and TOV1 flag will update on reaching OCR1
		   SET_BIT(TCCR1A,WGM10);								// MODE 15 //
		   SET_BIT(TCCR1A,WGM11);
		   SET_BIT(TCCR1B,WGM12);
		   SET_BIT(TCCR1B,WGM13);
	       break                ;
	...
	switch(ModelA){
		...
		case   Toggle :
			...
			CLR_BIT(TCCR1A,COM1A1);
		    break                 ;
		case   CLEAR :					// OC1A  will set at TOP (19999) //
			CLR_BIT(TCCR1A,COM1A0);
	...
	switch(ModelB){
		case   Normal :
		   CLR_BIT(TCCR1A,COM1B0);		// OCB is disconnected //
		...
		case   Toggle :
			...
			CLR_BIT(TCCR1A,COM1B1);
			break                 ;
		case   CLEAR :					// OC1B will set at TOP (OCR1A) and clear on OCRsB
